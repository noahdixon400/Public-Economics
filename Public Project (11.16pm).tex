...
\title{Distributional Resampling: an
...
There are five areas of literature that need to examined: (i) optimal experimental design, (ii) policy learning, (iii) transportability, and (iv) covariate resampling. %\textbf{It is not clear that (i) and (ii) will be valuable. (iv) is entirely contained within the computer science literature.}
...
In an implicitly canonical study, Hotz, Imbens, and Mortimer (2004) digress into the econometrics of transportability by evaluating empirical evidence from job training programs. Pearl and Bareinboim (2011; 2014) describe the formal conditions under which the results from one environment can be \emph{transported} into another environment. \emph{Proving that our algorithm satisfies the \enquote{do-algebra} proposed in this canonical study is essential to showing its statistical efficiency.} Recently, Wang, Han, and Huang (2024) develop an ongoing-sampling framework that tracks representatives over time to keep results externally valid when underlying user characteristics shift.
 

\subsection{Covariate Resampling}
Historically, propensity scores have been used to balance covariates \emph{within} sample (Rosenbaum \& Rubin, 1983). However, an increasingly important question is whether propensity scores can be used to balance covariates \emph{among} samples (Tipton 2013, Stuart et al. 2011, Dahabreh \& Hern\'an 2019). Adjaho and Christensen (2023) describe the conditions under which an individualized treatment policy -- like that proposed in Athey and Wager (2021) -- from one dataset (via an experiment or observational data) performs well when applied to a different population.

One issue, of course, is that propensity scores do not make treatment effects externally valid. The core of our algorithm involves the concept of a \enquote{synthetic sample,} or one that reconstructs a distribution based on the covariates in an external population. Currently, a synthetic sample is only described in the computer science literature, although it is attempting to solve a completely different problem. 
...
Suppose we have two populations, one of which is experimented on and the other of which is not experimented on. Also, assume that a subset of the covariates collected in the experiment population (age, gender, income, race, etc.) are known in the non-experimented population (via census data, survey, etc.). The idea is to use the results of the experimented population to infer the treatment effect in the non-experimented population. Fundamentally, the issue is that the underlying \emph{distribution} in the non-experimented population is different than the underlying distribution in the experimented population. Prior approaches in the literature have attempted to solve this using propensity scores. This method breaks down, however, especially in circumstances where support overlap is weak. Our algorithm constructs a \enquote{synthetic sample} that adjusts for these distributional differences. Moreover, under a certain set of assumptions, our algorithm may have \emph{economic meaning.} That is to say, after adjusting for distributional differences, we can ascertain the causal impact in the non-experimented population. Further, we can conduct econometric analysis, independent of the experimented population, implying that the causal impact of treatment in the experimented population and the non-experimented population can differ. This is particularly interesting to researchers thinking about \enquote{scalability.}
...
The following assumpitons are required: 
\begin{enumerate}
\item Unbiasedness in ATE in $p$. 
\item Consistency of ATE in $p$.
\item Homogeneity in treatment mechanism. For instance, in $p'$, the treatment effect will \emph{not} be produced by an entirely new mechanism. \emph{This is not something we must test.} Implicitly, researchers have this conceptualization in mind when they make policy recommendations. Structural models can be useful, in attempting to justify this assumption. 
\item We need some restrictions on covariate overlap, although it is unclear what exactly those restrictions are. 
\end{enumerate}
...
\subsubsection{Estimating the Distribution}

There are multiple cases we need to consider. The first occurs when we we have actual covariate values from $p'$. For instance, survey data gives us age, income, and gender for 100 individuals --  which we believe can be extrapolated to the larger population. The second occurs when we have\enquote{rough} covariates values from $p'$; that is to say, we know the median values, the upper and lower bounds, and rough categorical estimates. For instance, we know median individual is a 28-year-old male making $\$40,000$ a year, and we have rough estimates of the other categories (i.e., $20\%$ of individuals are females aged $30-50$ and $10\%$ of those individuals are making $\$48,000$ a year). 

\textbf{Case 1: Exact Covariates}
\\
Define \enquote{exact} covariates as those in which the covariates are defined by actual samples in the population. Define \enquote{inexact} covariates as those in which the covariates are defined by relativistic proportions in the population.

To estimate the target-population distribution $G_{p'}$, we use the ECDF (Empirical Cumulative Distribution Function). Let $X'_1,\dots,X'_{n'} \in \mathbb{R}^d$ denote the observed covariate vectors in population $p'$. The ECDF places probability mass $1/n'$ on each observed vector:
\[
\hat{G}_{p'}(x)
=
\frac{1}{n'} \sum_{i=1}^{n'}
\mathbf{1}( X'_{i1} \le x_1,\dots, X'_{id} \le x_d ).
\]
For instance, if there are three covariates, it looks like a set of jagged steps in $\rr^3$. 

Fundamentally, this estimator is nonparametric, preserves the empirical support of the covariates, and can accommodate continuous, discrete, and mixed-type variables. Further, it satisfies: 
\begin{enumerate}
\item \emph{Consistency}: $\hat{G}_{p'}(x) \overset{p}{\to} G_{p'}(x)$.
\item \emph{Unbiasedness}: $\mathbb{E}[\hat{G}_{p'}(x)] = G_{p'}(x)$.
\end{enumerate}

Note that this method uses the exact data; that is to say, it does not interpolate between data points. This is useful if we have a random sample of covariates from $p'$. However, it does require some strong assumptions. 

We can also use a kernel CDF estimator, especially if we are worried about mild exterpolation.  

\textbf{Case 2: Inexact Covariates}
\\
There is a second case worth considering. Suppose that we have census-level data that gives us approximate values. If we are willing to make parametric assumptions, we may be able to recover the distribution. For instance, we assume $\text{Age} \sim \text{Truncated Normal}(\mu,\sigma^2)$, where $\mu$ is given as the median and $\sigma^2$ is estimated from data. It is truncated because we know the relative bounds from the census data. 

IPF (Iterative Proportional Fitting) is an alternative to parametric assumptions (Choupani \& Mamdoohi, 2016). 

Jutras-Dub\'e et al. (2024) propose a ML method for generating synthetic data for target populations when only the empirical marginal distributions are known. 

There are significantly more assumptions underlying this process. This might be distinct from this paper.



\subsubsection{Matching}
The biggest thing to worry about here is exterpolation; that is to say, there will always be covariates represented in the reconstructed data. 

\textbf{Method 1: Nearest Neighbor Matching}
...
To test our algorithmic process, we will use the Progresa data set. As shown in Skoufias et al. (2001), the Progresa data set contains rich household measures on 24,000 households from 506 villages throughout Mexico. The Progresa data set is merely a tool that allows us to test the validity of our algorithm, specifically because there are a large amount of clusters and we already know the ex-post outcome. For instance, in a certain cluster (which can be as large or small as you want it to be), we can \enquote{pretend} like it was non-experimented. Then, we use the results from an experimented population to predict the ATE (using the algorithm described below) in the non-experimented population, using covariates that a researcher would likely have had access to ex-ante. Since we already know the ATE in the non-experimented population, it allows us to test how accurate our algorithm’s predictions were.

 %Intriguingly, we could use this data set to attempt to predict the CATE in other places, \emph{particularly those in which the effects of cash transfers were negligible.}

%\textbf{AZ Comments:} I am just commenting on what was due 10/30 (the data section). For the final project, please expand this to be consistent with how data sections are written in academic journals. Describe what Progresa is, the data collection, the variables, the sample, etc. I think this is an interesting project, but it reads vague to me. It is not clear exactly what you are doing, and so it is hard to understand how the Progresa data/context serves your purposes. The goal of the data section is to communicate that as much as it is to provide necessary information.
