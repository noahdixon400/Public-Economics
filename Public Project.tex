...
\title{Distributional Resampling: an Algorithmic Approach to Generalization}
\author{
...
\subsubsection{Empirical CDF (ECDF)}
To estimate $G_{p'}$, we can utilize any method for estimating the CDF. For instance, consider the univariate discrete case; its ECDF, $\hat{G}_n(x) = \frac{1}{n}\mathbf{1}(X_i \leq x)$, essentially creates a \enquote{staircase} that increases by $\frac{1}{n}$ at each observed data point. More specifically, it has the following properties: 
\begin{enumerate}
\item $\hat{G_n}(x) \overset{p}{\to} G(x)$.
\item $\mathbb{E}[\hat{G_n}(x)] = G(x)$.
\item $\text{Var}[\hat{G_n}(x)] = \frac{1}{n}G(x)[1-G(x)]$.
\end{enumerate}
For the multivariate case, we can use a kernel CDF estimator: 
\[
\hat{G}_h(x) = \frac{1}{n} \sum_{i=1}^n \prod_{j=1}^d \Phi\!\left(\frac{x_j - X_{ij}}{h_j}\right).
\]

\subsubsection{Nearest Neighbor Matching (and alternative methods)}
...
\begin{theorem}
When support overlap is weak, propensity score matching gives a biased estimate.
\end{theorem}
\begin{proof}
...
\textbf{Note that I will update this section based on Dr. Ziff's feedback for 11/20/25.}
